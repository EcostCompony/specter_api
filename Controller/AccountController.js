...
exports.edit = async (req, res) => {
	...
	try {
		...
		if (name && name.length > 64 || short_link && !short_link.match(/^[a-z\d\_\.]{4,32}$/i)) {
			let error_details = []
			if (name && name.length > 64) error_details.push({ "key": 'name', "value": name, "regex": '/^.{1,64}$/' })
			if (short_link && !short_link.match(/^[a-z\d\_\.]{4,32}$/i)) error_details.push({ "key": 'short_link', "value": short_link, "regex": '/^[a-z\\d\\_\\.]{4,32}$/i' })
