...
module.exports = async (req, res) => {
	...
	try {
		...
		if (name.length > 16 || !short_link.match(/^[a-z\d\_\.]{3,32}$/i)) {
			let error_details = []
			if (name.length > 16) error_details.push({ "key": 'name', "value": name, "regex": '/^.{1,16}$/' })
			if (!short_link.match(/^[a-z\d\_\.]{3,32}$/i)) error_details.push({ "key": 'short_link', "value": short_link, "regex": '/^[a-z\\d\\_\\.]{3,32}$/i' })
